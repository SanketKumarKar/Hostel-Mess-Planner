...
const AnnouncementsPage = () => {
        ...
        const fetchAnnouncements = async () => {
            ...
            try {
                ...
                if (profile.role === 'student') {
                    if (!profile.assigned_caterer_id || !profile.mess_type) { setAnnouncements([]); setLoading(false); return; }
                    query = query.eq('caterer_id', profile.assigned_caterer_id).in('mess_type', [profile.mess_type, 'all']);
    ...
    if (loading) {
        return (<div className="flex justify-center items-center h-64"><div className="text-gray-500">Loading announcements...</div></div>);
    }
            ...
            <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-4 sm:p-6 text-white shadow-lg relative overflow-hidden">
                <div className="relative z-10">
                    <h2 className="text-2xl sm:text-3xl font-bold mb-2 flex items-center gap-2 sm:gap-3">
                        <Megaphone className="w-6 h-6 sm:w-8 sm:h-8" /> Announcements
                    </h2>
                    <p className="opacity-90 max-w-xl text-sm sm:text-base">Stay updated with the latest news, menu changes, and special events from your mess caterer.</p>
                </div>
            </div>
