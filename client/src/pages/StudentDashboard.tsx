    ...
    const fetchActiveSession = async () => {
        try {
            const { data, error } = await supabase
                ...
                .select('*')
                .in('status', ['open_for_voting', 'finalized'])
        ...
        <div className="space-y-6">
            <div className="bg-gradient-to-r from-primary to-secondary rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
                <div className="relative z-10">
                    <h2 className="text-2xl font-bold mb-2">{activeSession.title}</h2>
                    <p className="opacity-90 flex items-center gap-2">
                        ...
                        {activeSession.status === 'finalized' ? (
                            ...
                            <span>Voting Open: {new Date(activeSession.start_date).toLocaleDateString()} - {new Date(activeSession.end_date).toLocaleDateString()}</span>
                        )}
                    </p>
                    {profile?.mess_type && (
                        <div className="mt-4 flex flex-wrap items-center gap-3">
                            ...
                            {activeSession.status === 'open_for_voting' && (
                                <button
            ...
            {activeSession.status === 'finalized' ? (
                <FinalMenuDisplay session={activeSession} />
            ) : (
                <VotingInterface session={activeSession} onEditProfile={() => setShowProfileEdit(true)} />
            )}
...
const ProfileEditor = ({ onClose }: { onClose: () => void }) => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            alert('Mess type updated! Please refresh to see changes.');
