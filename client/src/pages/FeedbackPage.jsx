...
const FeedbackPage = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const { data: feedbackData } = await supabase.from('feedbacks').select('*, caterer:profiles!caterer_id(full_name)').eq('student_id', profile.id).order('created_at', { ascending: false });
    ...
    const handleSubmit = async (e) => {
        ...
        setSubmitting(true);
        try {
            const { error } = await supabase.from('feedbacks').insert({ student_id: profile.id, caterer_id: selectedCaterer, message: message.trim() });
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 h-fit">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <select value={selectedCaterer} onChange={(e) => setSelectedCaterer(e.target.value)} className="w-full rounded-lg border-gray-300 focus:ring-primary focus:border-primary p-2.5 bg-gray-50 hover:bg-white transition-colors border" required>
                        ...
                        <div>
                            ...
                            <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={5} className="w-full rounded-lg border-gray-300 focus:ring-primary focus:border-primary p-3 bg-gray-50 hover:bg-white transition-colors border resize-none" placeholder="Write your compliments or complaints here..." required />
                        </div>
                        <button type="submit" disabled={submitting || !selectedCaterer || !message} className={`w-full py-2.5 rounded-lg flex items-center justify-center gap-2 font-bold text-white transition-all ${submitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-indigo-700 hover:shadow-md'}`}>
                        ...
                        {feedbacks.length === 0 ? (
                            ...
                            feedbacks.map((item) => (
                                <div key={item.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                    ...
                                    <div className="bg-gray-50 p-3 rounded-lg text-gray-700 text-sm mb-3 border border-gray-100">"{item.message}"</div>
                                    {item.response ? (
